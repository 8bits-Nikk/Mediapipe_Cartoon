...
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
    while cap.isOpened():
        ...
        try:
            ...
            head = [WIDTH * landmarks[mp_pose.PoseLandmark.NOSE.value].x,
                    HEIGHT * landmarks[mp_pose.PoseLandmark.NOSE.value].y]

            R_hand_mid = [WIDTH * landmarks[mp_pose.PoseLandmark.LEFT_ELBOW].x,
                          HEIGHT * landmarks[mp_pose.PoseLandmark.LEFT_ELBOW.value].y]
            R_hand_end = [WIDTH * landmarks[mp_pose.PoseLandmark.LEFT_WRIST.value].x,
                          HEIGHT * landmarks[mp_pose.PoseLandmark.LEFT_WRIST.value].y]

            L_hand_mid = [WIDTH * landmarks[mp_pose.PoseLandmark.RIGHT_ELBOW].x,
                          HEIGHT * landmarks[mp_pose.PoseLandmark.RIGHT_ELBOW.value].y]
            L_hand_end = [WIDTH * landmarks[mp_pose.PoseLandmark.RIGHT_WRIST.value].x,
                          HEIGHT * landmarks[mp_pose.PoseLandmark.RIGHT_WRIST.value].y]

            R_leg_mid = [WIDTH * landmarks[mp_pose.PoseLandmark.LEFT_KNEE].x,
                         HEIGHT * landmarks[mp_pose.PoseLandmark.LEFT_KNEE.value].y]
            R_leg_end = [WIDTH * landmarks[mp_pose.PoseLandmark.LEFT_ANKLE.value].x,
                         HEIGHT * landmarks[mp_pose.PoseLandmark.LEFT_ANKLE.value].y]

            L_leg_mid = [WIDTH * landmarks[mp_pose.PoseLandmark.RIGHT_KNEE].x,
                         HEIGHT * landmarks[mp_pose.PoseLandmark.RIGHT_KNEE.value].y]
            L_leg_end = [WIDTH * landmarks[mp_pose.PoseLandmark.RIGHT_ANKLE.value].x,
